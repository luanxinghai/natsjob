...
export const useTable = (
  ...
  const useTableRemoveById = async (
    options = { id, path },
    callbackSuccess = useTableSearch,
    callbackFail,
  ) => {
    let params
    //如果是对象
   ...
   * @param {*} callbackFail
   */
  const useTableRemoveByIds = async (
    options = { ids: [], path: "" },
    callbackSuccess = useTableSearch,
    callbackFail,
  ) => {
  ...
  const useTableGetFieldDownload = async (field = [], fieldData = []) => {
    let queryParms = getSearchForm()
    queryParms["field"] = field
    queryParms["fieldData"] = fieldData

    await $$.download(
      `${config.api}/${apiPath.download}`,
      queryParms,
      `${config.excelName}`,
    )
  }
