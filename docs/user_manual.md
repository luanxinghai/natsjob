...
| 类型          | Subject 示例                          | 功能说明                                   |
|---------------|---------------------------------------|--------------------------------------------|
| PreStart      | `natsjob.job.pre-start.app.biz.owner`<br />`natsjob-void.job.pre-start.app.biz.owner` | 任务预启动（Map 模式前置流程）             |
| Start         | `natsjob.job.start.app.biz.owner`<br />`natsjob-void.job.start.app.biz.owner` | 任务启动（单机/广播核心场景）              |
| ClientStart   | `natsjob.job.client-start.app.biz.owner`<br />`natsjob-void.job.client-start.app.biz.owner` | 客户端定向启动（支持按客户端 ID 分发）    |
| GroupStart    | `natsjob.job.group-start.app.biz.owner`<br />`natsjob-void.job.group-start.app.biz.owner` | 分组启动（功能尚未实现）                   |
| AgentStart    | `natsjob.job.agent-start.app.biz.owner`<br />`natsjob-void.job.agent-start.app.biz.owner` | 智能体启动（功能尚未实现）                 |
| SubResultFlow | `natsjob.job.sub-result-flow` | 子任务流水信息上报subject通道（客户端发起）         |
| Monitor       | `natsjob.job.monitor.{namespace}.{appName}`    | 监控subject（客户端监听它自定义处理后续监控逻辑）       |
...
| 等级         | Subject 示例                          | 心跳                        | 写库                        | 并发超时                    | 核心特性                                                                 |
|:------------:|:--------------------------------------|:------------------------------------------------------------------------:|:------------------------------------------------------------------------:|--------------------------------------------------------------------------|--------------------------------------------------------------------------|
| 轻量-plus    | `natsjob.job.start.app.biz.owner`<br />`natsjob-void.job.start.app.biz.owner` | 否 | 否 | 否 | 性能怪兽 |
| 基础-max     | 同上 | 否 | 是 | 是 | 无心跳可下发任务 |
| 专业-pro     | 同上 | 是 | 是 | 是 | 无心跳不下发任务 |
| 客户端-ultra   | `natsjob.job.client-start.app.biz.owner.<客户端ID>` | 是 | 是 | 是 | 无心跳不下发任务。<br />可按「随机/轮询/最先注册/最大权重」策略定向下发任务 |
...
| 基础-max     | 同上    | 否   | 是   | 是  | 无心跳可下发任务 |
| 专业-pro     | 同上    | 是 | 是   | 是  | 无心跳不下发任务 |
| 客户端-ultra   | `natsjob.job.client-start.app.biz.owner.<客户端ID>` | 是 | 是 | 是 | 无心跳不下发任务。<br />按已上报心跳的客户端 ID 批量下发任务 |

## 四、Map 模式

Map 模式适配 Map/MapReduce 等流程化任务场景，核心逻辑为「预启动任务 → 客户端回复 → 触发下一步/结束任务」，natsjob 仅等待最终结果，流程控制由客户端自主实现。

| 等级         | Subject 示例                          | 心跳                        | 写库                        | 并发 超时                   | 核心特性                                                                 |
|--------------|---------------------------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------|
| 轻量-plus    | `natsjob.job.pre-start.app.biz.owner`<br />`natsjob-void.job.pre-start.app.biz.owner` | 否 | 否 | 否 | 性能怪兽 |
| 基础-max     | 同上 | 否 | 是 | 是 | 无心跳可下发任务<br />▸ 客户端回复 `success:next`：natsjob 下发下一步任务（Subject：`natsjob.job.start.app.biz.owner`）<br/>▸ 客户端回复 `success/fail`：任务彻底结束，natsjob关闭该任务 |
| 专业-pro     | 同上 | 是 | 是 | 是 | 无心跳则不处理任务<br/>客户端回复逻辑同「max」等级 |
...
### MAP/MapReduce 自定义设计思路
natsjob 仅提供基础的任务下发与结果接收能力，不介入 Map/MapReduce 流程的具体实现，客户端可完全自定义分片、处理、流转逻辑，核心设计思路如下：
...
- **监控主题**：`natsjob.job.monitor.{namespace}.{appName}`

### 工作流程
1. 客户端上报任务结果时，若 `monitorStatus` 不为空时触发监控下发
2. natsjob 会自动将监控消息推送到 `natsjob.job.monitor.{namespace}.{appName}`

   下发示例: 下发subject为`natsjob.job.monitor.app.biz`

   监听示例:
   - `natsjob.job.monitor.app.biz` 监听具体应用的监控消息
   - `natsjob.job.monitor.app.*` 监听具体应用下的所有监控消息
   - `natsjob.job.monitor.*.*` 监听所有命名空间下所有应用的监控消息
...
即使你在 natsjob 管理页面配置的是单机任务，只要客户端按广播、Map 模式去处理消息，一样可以正常运行——框架不做强制限制，真正做到**轻量、可控、面向场景**。祖-ultra   | `natsjob.job.client-start.app.biz.owner.<客户端ID>` | 是 | 是 | 是 | 无心跳不下发任务。<br />按已上报心跳的客户端 ID 批量下发任务 |
