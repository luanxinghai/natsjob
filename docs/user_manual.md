...
|:------------:|:--------------------------------------|:------------------------------------------------------------------------:|:------------------------------------------------------------------------:|--------------------------------------------------------------------------|--------------------------------------------------------------------------|
| 筑基-plus    | `natsjob.job.start.app.biz.owner`<br />`natsjob-void.job.start.app.biz.owner` | 否 | 否 | 否 | 性能怪兽 |
| 金丹-max     | 同上 | 否 | 是 | 是 | 无心跳可下发任务 |
| 元婴-pro     | 同上 | 是 | 是 | 是 | 无心跳不下发任务 |
| 道祖-ultra   | `natsjob.job.client-start.app.biz.owner.<客户端ID>` | 是 | 是 | 是 | 无心跳不下发任务。<br />可按「随机/轮询/最先注册/最大权重」策略定向下发任务 |
...
| 金丹-max     | 同上    | 否   | 是   | 是  | 无心跳可下发任务 |
| 元婴-pro     | 同上    | 是 | 是   | 是  | 无心跳不下发任务 |
| 道祖-ultra   | `natsjob.job.client-start.app.biz.owner.<客户端ID>` | 是 | 是 | 是 | 无心跳不下发任务。<br />按已上报心跳的客户端 ID 批量下发任务 |
...
|--------------|---------------------------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------|
| 筑基-plus    | `natsjob.job.pre-start.app.biz.owner`<br />`natsjob-void.job.pre-start.app.biz.owner` | 否 | 否 | 否 | 性能怪兽 |
| 金丹-max     | 同上 | 否 | 是 | 是 | 无心跳可下发任务<br />▸ 客户端回复 `success:next`：natsjob 下发下一步任务（Subject：`natsjob.job.start.app.biz.owner`）<br/>▸ 客户端回复 `success/fail`：任务彻底结束，natsjob关闭该任务 |
| 元婴-pro     | 同上 | 是 | 是 | 是 | 无心跳则不处理任务<br/>客户端回复逻辑同「max」等级 |
