...
func runJobResult(task Task) {
	...
	if err != nil {
		logger.Error("pre-end get jobKey fail",
	...
	if err != nil {
		logger.Error("pre-end unmarshal jobKey fail",
			zap.String("jobKey", jobKey),
			zap.String("jobEndKey", jobEndKey),
			zap.Error(err),
		)
	...
	cache.Set(jobKey, jobEndKey, subDuration)
	logger.Info("pre-end jobResults kv  test history add cache",
