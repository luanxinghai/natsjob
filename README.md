...
| NATSJOB_NATS_URL                 | "nats://127.0.0.1:4222" | NATS 服务连接地址，带账号密码时格式为：nats://用户名:密码@IP:端口 - 示例：nats://natsjob:natsjob123@127.0.0.1:4222                                                                                                                                                                                                                                 |
...
| NATSJOB_DB_TYPE                  | "sqlite"                | 数据库类型，支持 - sqlite- mysql- postgres- dm（达梦）                                                                                                                                                                                                                                                                              |
| NATSJOB_DB_URL                   | ""                      | 数据库连接地址：- sqlite 可留空（默认使用本地文件）- mysql 格式：root:natsjob123@tcp(127.0.0.1:3306)/natsjob?charset=utf8mb4&parseTime=True&loc=Local- postgres 格式：host=127.0.0.1 user=postgres password=natsjob123 port=5432 dbname=natsjob sslmode=disable TimeZone=Asia/Shanghai- 达梦 格式：dm://SYSDBA:natsjob123@127.0.0.1:5236?schema=natsjob |
...
| 10000 次/10秒（长时间运行服务器资源全打满，数据库CPU较高，访问出现异常） | 内存 < 400M、CPU < 600% | 内存 < 600M、CPU < 200% | 1s       | 10   | 10s  | 100 / 1000（对比测试） |
...
你也可以使用skill生成代码,前面有skills路径文件

```
你读取这个skill 给我一个java  单机版 max场景的代码示例
```
